...
function initializeSocket(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('consultation:join', ({ consultationId }) => {
      ...
      // Store consultationId on socket for later use
    ...
    // WebRTC Offer: forward from caller to callee
    socket.on('webrtc:offer', ({ consultationId, offer, targetUserId }) => {
      ...
      // Send to specific user in the consultation room
    ...
    // WebRTC Answer: forward from callee back to caller
    ...
    // WebRTC ICE Candidate: forward to peer
